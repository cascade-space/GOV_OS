...
export default function ConstituencyPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
            ...
            <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-blue-950 text-white py-12 border-b border-slate-800">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="grid grid-cols-2 gap-3 self-start md:self-auto text-xs">
                            <div className="bg-slate-800/80 p-3 rounded-xl border border-slate-700/80 text-center">
                                <p className="text-slate-400 font-medium">Covered Wards</p>
                                <p className="text-lg font-bold text-white">8 Wards</p>
                            </div>
                            <div className="bg-slate-800/80 p-3 rounded-xl border border-slate-700/80 text-center">
                                <p className="text-slate-400 font-medium">Population Impact</p>
                                <p className="text-lg font-bold text-emerald-400">280,000+</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 space-y-12">
                ...
                <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2">
                        <div>
                            <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">Sectoral Transformation</span>
                            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                                Improvements Across Dharwad
                            </h2>
                        </div>
                        <p className="text-xs text-slate-500">Aggregated infrastructure and civic health across key public domains</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        {sectors.map((sector) => {
                            ...
                            return (
                                <div
                                    key={sector.id}
                                    className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm hover:shadow-md transition duration-200 space-y-4"
                                >
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-900 text-base">{sector.name}</h3>
                                        <div className="mt-2 grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-100">
                                            <div>
                                                <span className="text-slate-400 font-medium">Resolved</span>
                                                <p className="text-sm font-bold text-emerald-600">{sector.resolved} items</p>
                                            </div>
                                            <div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <span className="text-xs font-bold text-emerald-600 uppercase tracking-wider">Ward-Level Accountability</span>
                            <h2 className="text-xl font-bold text-slate-900">Ward Performance Matrix</h2>
                            <p className="text-xs text-slate-500">Public aggregated completion and grievance closure rate across 8 wards</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {DHARWAD_WARDS.map((w) => {
                            ...
                            return (
                                <div
                                    key={w.id}
                                    className="p-4 rounded-xl bg-slate-50 border border-slate-200/80 hover:bg-slate-100/70 transition space-y-3"
                                >
                                    <div className="flex items-center justify-between">
                                    ...
                                    <h4 className="text-xs font-bold text-slate-800 leading-snug line-clamp-1">{w.name}</h4>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex justify-between text-[11px] text-slate-500 pt-0.5">
                                            <span>Total: <strong>{w.totalIssues}</strong></span>
                                            <span>Closed: <strong className="text-emerald-700">{w.resolved}</strong></span>
                                        </div>
                                    </div>
                                </div>
                            );
                ...
                <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                        <div>
                            <span className="text-xs font-bold text-amber-600 uppercase tracking-wider">Live Municipal Execution</span>
                            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                                Government Works in Progress
                            </h2>
                            <p className="text-xs text-slate-500">Major infrastructure and public projects scheduled across Dharwad</p>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                className="bg-white border border-slate-300 text-slate-700 text-xs font-semibold rounded-xl px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
